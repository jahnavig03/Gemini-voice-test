...
function formatGeminiUserError(err) {
    ...
    if (/403/.test(raw) &&
        /reported as leaked|was leaked|invalid api key|api key invalid|api key not valid|incorrect api key|wrong api key/i.test(raw)) {
        return {
            message: "Gemini rejected the API key (invalid, revoked, or reported as leaked). Create a **new** key in Google AI Studio and update GEMINI_API_KEY in Railway Variables — do not reuse a key that was ever committed or pasted publicly.",
            code: "GEMINI_API_KEY",
        };
    }
    ...
    if (/403/.test(raw) || /\bforbidden\b/i.test(lower)) {
        return {
            message: "Gemini returned Forbidden — check the API key, billing, and that the model is enabled for your project.",
