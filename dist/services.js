...
exports.ElevenLabsService = exports.GeminiService = void 0;
exports.isEnglishLatinCaption = isEnglishLatinCaption;
exports.assertAsciiEnglishUserCaption = assertAsciiEnglishUserCaption;
// src/services.ts
...
function isEnglishLatinCaption(s) {
    const t = s.trim();
    if (!t)
        return false;
    if (captionContainsBlockedScript(t))
        return false;
...
function assertAsciiEnglishUserCaption(s) {
    const t = s.trim();
    if (!t)
        return t;
...
class GeminiService {
    ...
    async welcomeChatOpening() {
        ...
        const result = await model.generateContent({
            contents: [{ role: "user", parts: [{ text: prompts_1.OPENING_GREET_USER_TURN }] }],
            generationConfig: {
                maxOutputTokens: 512,
                temperature: 0.75,
                topP: 0.9,
    ...
    static concatGenerateContentText(result) {
        try {
    ...
    async englishLiveCaption(raw) {
        ...
        const loose = "You normalize voice-assistant chat captions for an English-only chat log.\n\n" +
            ...
            "Raw caption:\n" +
            t;
        const strict = "The input uses non-Latin scripts or wrong script for English speech. Output exactly ONE line.\n" +
     ...
     */
    async englishBotTranscriptDisplay(raw) {
        const t = raw.trim();
        if (!t)
            return t;
        if (/^[\x20-\x7E]+$/.test(t) && isEnglishLatinCaption(t)) {
        ...
        const loose = "Below is a speech-to-text transcript of what an IPL voice assistant said aloud to the user.\n" +
            ...
            "Transcript:\n" +
            t;
        const strict = "The transcript uses non-Latin scripts or mixed text. Output exactly ONE line: a faithful English translation.\n" +
